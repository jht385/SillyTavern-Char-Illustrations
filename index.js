...
function openImagePickerModal(item, onSelect) {
    ...
    const itemsHtml = imgs.map((src, idx) => {
        ...
        <div class="ci-panel">
            ...
            <div class="ci-items-list">
                ...
                ${filteredItems.map((item, index) => {
                    ...
                    const previewSrc = imgs[selectedIdx] || imgs[0] || '';
                    const names = item.imageNames || [];
                    ...
                    <div class="ci-card" data-id="${item.id}">
                        <div class="ci-thumb-box">
                            <div class="ci-thumb-preview-wrap" data-id="${item.id}" title="点击放大主图 (共 ${imgs.length} 张图片)">
                                <img class="ci-thumb-main" src="${previewSrc}" loading="lazy" onclick="window.openCIModal('${previewSrc}')" />
...
async function checkAndRenderMessage(messageEl) {
    ...
    listToDisplay.forEach(item => {
        ...
        if (singleImageMode) {
            ...
            const currentImgSrc = imgs[selectedIdx];
            ...
            img.onclick = () => window.openCIModal(img.src);
            ...
            imgs.forEach(imgSrc => {
                ...
                img.loading = 'lazy';
                img.title = `点击放大查看\n触发规则: ${formatRuleDisplay(item)}`;
                img.onclick = () => window.openCIModal(imgSrc);
