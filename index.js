...
// 现代浏览器直接静默复制文本，避免触发手机端 prompt 弹窗
    ...
    if (deleteAllBtn) {
        deleteAllBtn.onclick = (e) => {
            ...
            showGlobalDialog(
                ...
                async () => {
                    await dbDeleteAllByChar(charId);
                    localStorage.removeItem(`ci_groups_${charId}`); // 同步清空该角色的自定义分组
                    clearOldIllustrations();
                    await renderSettings(true);
                    debouncedScan();
                }
