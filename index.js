...
// ==================== 0. Blob / ObjectURL 核心生命周期与内存管理器 ====================
class BlobUrlManager {
    constructor(maxCacheSize = 250) {
        this.maxCacheSize = maxCacheSize;
        this.blobMap = new WeakMap();
        this.cacheMap = new Map(); // base64 string -> objectUrl
    }
    ...
    getUrl(source) {
        ...
        if (typeof source === 'string') {
            ...
            if (source.startsWith('data:image')) {
                if (this.cacheMap.has(source)) {
                    // LRU 更新
                    const url = this.cacheMap.get(source);
                    this.cacheMap.delete(source);
                    this.cacheMap.set(source, url);
                    return url;
                }
                try {
                    ...
                    if (this.cacheMap.size >= this.maxCacheSize) {
                        ...
                        URL.revokeObjectURL(this.cacheMap.get(oldestKey));
                        this.cacheMap.delete(oldestKey);
                    }
                    this.cacheMap.set(source, url);
                    return url;
                } catch (e) {
                    console.error('[插图插件] Base64 转 Blob 失败:', e);
                    return source;
                }
            }
        }
    ...
    clearStringCache() {
        for (const url of this.cacheMap.values()) {
...
const urlManager = new BlobUrlManager(200);
...
// ==================== 2. JSON 自动优化与清理管道 (画质完全无损) ====================
/**
 * 清洗规则：去除重复项、清理空格、过滤空字符串
 */
...
/**
 * 标准化插图对象（转 Blob、清除冗余属性、保持 100% 原始画质）
 */
...
async function dbSaveOptimizedRecord(charId, rawItem, fallbackOrder = 1) {
    const optimized = optimizeIllustrationRecord(rawItem, fallbackOrder);
    if (!optimized) return;
                ...
                if (char === '{') {
                    ...
                    if (depth === 0 && currentObjectStart !== -1) {
                        ...
                        try {
                            ...
                            importedCount++;
                            await dbSaveOptimizedRecord(charId, parsedItem, importedCount);

                            const mbLoaded = (loadedBytes / 1024 / 1024).toFixed(1);
                            const mbTotal = (totalBytes / 1024 / 1024).toFixed(1);
                            if (onProgress) {
                                onProgress(loadedBytes, totalBytes, `流式解析与自动优化中 (${mbLoaded}MB / ${mbTotal}MB) · 已导入 ${importedCount} 项`);
                            }
                        } catch (err) {
                            console.warn('[插图插件] 忽略损坏项:', err);
                        }
        ...
        return `
            <div class="ci-picker-card ${isActive ? 'active' : ''}" data-idx="${idx}">
                <div class="ci-picker-img-box">
                    <img class="ci-lazy-img" data-src="${url}" decoding="async" />
...
async function renderSettings() {
        ...
        <div class="ci-card" data-id="${item.id}">
            <div class="ci-thumb-box">
                <div class="ci-thumb-preview-wrap" data-id="${item.id}" title="点击放大主图 (共 ${imgs.length} 张图片)">
                    <img class="ci-thumb-main ci-lazy-img" data-src="${displayUrl}" decoding="async" onclick="window.openCIModal('${displayUrl}')" />
    ...
    container.innerHTML = `
        <div class="ci-panel">
            ...
            <div class="ci-upload-box">
                <b>为角色 [${charName}] 添加新规则 (原生 Blob 极速存储)</b>
            ...
            <div class="ci-list-header">
                ...
                <div class="ci-header-actions">
                    ...
                    <button id="ci-import-btn" class="menu_button ci-mini-btn" title="导入配置 (自动清洗优化)">导入</button>
    ...
    document.getElementById('ci-export-btn').onclick = async () => {
        ...
        progress.update(0, total, `[2/2] ⚡ 正在无损生成标准化 JSON (0/${total})`);
        ...
        for (let i = 0; i < total; i++) {
            ...
            progress.update(i + 1, total, `[2/2] ⚡ 正在无损生成标准化 JSON (${i + 1}/${total})`);
        ...
        a.href = url;
        a.download = `${charName}_插图配置_优化版.json`;
        a.click();
        URL.revokeObjectURL(url);
    ...
    importInput.onchange = async (e) => {
        ...
        const progress = createProgressModal('📥 正在导入并自动优化配置');
        let importedCount = 0;

        try {
            progress.update(0, 100, '清理历史旧配置...');
            await dbDeleteAllByChar(charId);
            clearOldIllustrations();
            urlManager.clearStringCache();
            ...
            progress.update(100, 100, '重构界面显示中...');
            await renderSettings();
            debouncedScan();

            progress.update(100, 100, '🎉 导入与自动优化已全部完成！');
            progress.close(500);
            toastr?.success?.(`已导入并自动优化 ${importedCount} 项规则配置！`, '插图插件');
        } catch (err) {
            progress.close(0);
