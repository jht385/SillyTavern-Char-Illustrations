...
function base64ToBlob(base64) {
    ...
    try {
        ...
        console.warn('[插图插件] Base64 转 Blob 异常降级:', e);
        return base64;
    }
}
...
// ==================== 4. 增强 IndexedDB 存储引擎 (带别名容错查询，永不掉插图) ====================
        ...
        try {
            ...
            req.onsuccess = (e) => {
                ...
                if (cursor) {
                    ...
                    totalBytes += size;
                    if (item.charId === charId || aliases.includes(item.charId)) {
                        charBytes += size;
                    }
                    cursor.continue();
                } else {
...
// 核心优化：全别名容错查询，彻底消除切页面或名称轻微变更造成的“插图全部蒸发”
async function dbGetIllustrations(charId) {
    ...
    const db = await initDB();
    const candidateIds = getCharAliases(charId);
            ...
            candidateIds.forEach(cId => {
                ...
                req.onsuccess = () => {
                    if (Array.isArray(req.result)) {
                        accumulated = accumulated.concat(req.result);
                    }
                    processed++;
                    if (processed === candidateIds.length) {
                        // 去重
...
// 稳定获取角色标识与别名数组，彻底锁定唯一身份，永不掉插图
function getCharAliases(explicitCharId = null) {
    const context = getContext();
    const list = new Set();
    if (explicitCharId) list.add(String(explicitCharId));

    if (context?.characters && context.characterId !== undefined && context.characters[context.characterId]) {
        const char = context.characters[context.characterId];
        if (char.avatar) list.add(String(char.avatar));
        if (char.name) list.add(String(char.name));
        if (char.id !== undefined) list.add(String(char.id));
        list.add(String(context.characterId));
    }
...
function openMoveGroupDialog(charId, item, onUpdated) {
    ...
    modal.innerHTML = `
        <div class="ci-confirm-content" style="border-color:#48cae4 !important; max-width:380px;">
            <h3 style="color:#48cae4 !important; font-size:1.05em;">🏷️ 移动至分组 (ID: ${item.id})</h3>
        ...
        <div class="ci-card-info">
            <div class="ci-card-header-row">
                <div style="display:flex; align-items:center; gap:6px; overflow:hidden;">
                    <span style="font-size:0.8em; opacity:0.85; white-space:nowrap;">[ID: ${item.id}]</span>
                    ${groupBadge}
...
async function renderSettings(forceRefreshStats = false) {
    ...
    const folderBlocksHtml = groupsToDisplay.map(groupKey => {
        ...
        if (groupItems.length === 0 && (activeGroupFilter || searchFilterKeyword.trim())) {
            return ''; 
        }
        ...
        <div class="ci-panel">
            ...
            <div class="ci-list-header">
                ...
                <div class="ci-header-actions">
                    ...
                    <button type="button" id="ci-optimize-btn" class="menu_button ci-mini-btn" style="background:#3a86ff !important; color:#fff;" title="一键无损整理规则、去重并消除冗余字段">⚡ 优化库配置</button>
            ...
            <div class="ci-group-tabs-bar">
                ...
                ${allExistingGroups.map(g => `
                    <div class="ci-group-tab-item ${activeGroupFilter === g ? 'active' : ''}" data-g="${escapeHtml(g)}">${escapeHtml(g)} (${items.filter(i => i.groupName === g).length})</div>
                `).join('')}
            </div>
    ...
    if (optimizeBtn) {
        optimizeBtn.onclick = async (e) => {
            ...
            const count = await dbOptimizeCurrentChar(charId);
            urlManager.clearCache();
            await renderSettings();
            toastr?.success?.(`已无损优化 ${count} 条插图配置！`, '插图插件');
        };
    }
        ...
        deleteAllBtn.onclick = (e) => {
            ...
            showGlobalDialog(
                ...
                <p style="margin: 0; line-height: 1.5; font-size: 0.95em;">
                    您确定要<b>一键清空并删除</b>角色【${escapeHtml(charName)}】的所有插图数据吗？<br>
...
function setupIllustrationCarousel(containerEl, charId) {
    ...
    // 移动端动态挪移按键到底部指示条两侧，100%不遮挡图片
    const isMobile = window.innerWidth <= 768;
    if (isMobile && footer && prevBtn && nextBtn) {
        if (!footer.contains(prevBtn)) {
            footer.prepend(prevBtn);
            footer.appendChild(nextBtn);
        }
    } else if (!isMobile && prevBtn && nextBtn) {
        const wrapper = containerEl.querySelector('.ci-carousel-wrapper');
        const viewport = containerEl.querySelector('.ci-carousel-viewport');
        if (wrapper && viewport && !wrapper.contains(prevBtn)) {
...
async function checkAndRenderMessage(messageEl) {
    ...
    footer.innerHTML = `
        <span class="ci-carousel-badge">1 / 1</span>
    ...
    for (const item of listToDisplay) {
        ...
        if (singleImageMode) {
            ...
            // 核心修复：Blob URL 自愈与重新挂载，彻底杜绝掉图白屏
            img.onerror = () => {
            ...
            if (imgs.length > 1) {
                ...
                tagWrap.className = 'ci-name-tag-wrapper';

                if (item.groupName) {
                    const groupPrefix = document.createElement('span');
                    groupPrefix.className = 'ci-switch-group-prefix';
                    groupPrefix.innerText = item.groupName;
                    tagWrap.appendChild(groupPrefix);
                }

                const nameLabel = document.createElement('span');
                nameLabel.className = 'ci-switch-name-label';
                nameLabel.innerText = displayName;
                nameLabel.title = `点击复制名称: "${displayName}"`;

                nameLabel.onclick = async (e) => {
                    e.stopPropagation();
                    const textToCopy = (nameLabel.dataset.rawName || displayName).trim();
                    try {
                        await navigator.clipboard.writeText(textToCopy);
                        const oldText = nameLabel.innerText;
                        nameLabel.innerText = '已复制 ✓';
                        setTimeout(() => {
                            nameLabel.innerText = oldText;
                        }, 1200);
                    } catch (_) {
                        prompt('请全选复制名称：', textToCopy);
                    }
                };
                ...
                switchBtn.onclick = (e) => {
                    ...
                    requestAnimationFrame(() => {
                        openImagePickerModal(item, (chosenIdx) => {
                            ...
                            const updatedName = ((item.imageNames || [])[chosenIdx] || '').trim();
                            const newDisplayName = updatedName || `插图 ${chosenIdx + 1}`;
                            nameLabel.innerText = newDisplayName;
                            nameLabel.dataset.rawName = newDisplayName;
                            nameLabel.title = `点击复制名称: "${newDisplayName}"`;
                ...
                switchWrap.appendChild(tagWrap);
                switchWrap.appendChild(switchBtn);
                innerCard.appendChild(switchWrap);
            }
...
jQuery(async () => {
    ...
    window.addEventListener('resize', debounce(() => {
        const container = document.querySelector('.char-illustration-container, #ci-top-fixed-container');
        if (container) {
            setupIllustrationCarousel(container, getCurrentCharId());
        }
    }, 200));
