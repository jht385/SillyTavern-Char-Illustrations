...
class ImageUrlManager {
    constructor(maxCache = 300) {
        this.maxCache = maxCache;
        this.cache = new Map(); // key -> objectUrl
    }
    ...
    getUrl(source) {
        ...
        if (typeof source === 'string') {
            ...
            if (this.cache.has(base64)) {
                const url = this.cache.get(base64);
                this.cache.delete(base64);
                this.cache.set(base64, url);
                return url;
            }
            try {
        ...
        if (source instanceof Blob) {
            try {
                ...
                console.error('[插图插件] Blob 生成 URL 失败:', e);
                return '';
            }
        }
        return '';
...
const urlManager = new ImageUrlManager(300);
...
/**
 * 高兼容 Base64 转 Blob 二进制
 */
function base64ToBlob(base64) {
    if (base64 instanceof Blob) return base64;
    const parts = base64.split(';base64,');
    const contentType = (parts[0] && parts[0].includes(':')) ? parts[0].split(':')[1] : 'image/png';
    const rawBase64 = parts[1] || parts[0];
    const raw = window.atob(rawBase64.replace(/[\r\n\s]/g, ''));
...
async function dbBatchInsertOptimized(charId, rawList) {
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let i = 0; i < rawList.length; i++) {
            const opt = optimizeIllustrationRecord(rawList[i], i + 1);
            if (opt) {
                store.add({
                    charId,
                    ...opt
    ...
    modal.innerHTML = `
        <div class="ci-confirm-content" style="max-width: 600px; width: 92%; max-height: 85vh; display: flex; flex-direction: column;">
            ...
            <div style="font-size: 0.88em; color: #eee; line-height: 1.5;">
                <p style="margin: 0 0 6px 0;">解析或写入数据库时发生异常。</p>
            </div>
...
/**
 * 稳健导入处理器（修复刷新消失和中断）：自动分块事务写入
 */
async function parseAndImportJsonFile(file, charId, onProgress) {
    const text = await file.text();
    let data;
    try {
        data = JSON.parse(text);
    } catch (e) {
        throw new Error('JSON 解析失败，文件格式可能损坏: ' + e.message);
    }

    let rawList = [];
    if (Array.isArray(data)) {
        rawList = data;
    } else if (data && Array.isArray(data.illustrations)) {
        rawList = data.illustrations;
    } else {
        throw new Error('未识别到有效的插图配置列表 (未找到 illustrations 数组)');
    }
    ...
    // 分批事务写入，避免大容量文件将移动端 IndexedDB 锁死
    const chunkSize = 25;
    let importedCount = 0;

    for (let i = 0; i < total; i += chunkSize) {
        const chunk = rawList.slice(i, i + chunkSize);
        const count = await dbBatchInsertOptimized(charId, chunk);
        importedCount += count;
        if (onProgress) {
            onProgress(importedCount, total, `正在写入数据库 (${importedCount}/${total})...`);
        }
        ...
        return `
            <div class="ci-picker-card ${isActive ? 'active' : ''}" data-idx="${idx}">
                <div class="ci-picker-img-box">
                    <img class="ci-lazy-img" data-src="${url}" src="${url}" decoding="async" />
            ...
            return `
                <div class="ci-manage-item ${isSelected ? 'is-selected' : ''}" data-idx="${idx}">
                    <div class="ci-manage-img-wrap">
                        ...
                        <img src="${url}" loading="lazy" decoding="async" title="点击放大预览" onclick="window.openCIModal('${url}')" />
                    </div>
...
async function renderSettings() {
    ...
    if (!charId) {
        container.innerHTML = `<div style="opacity:0.7; font-size:0.9em; padding:10px;">请先选择并打开一个角色对话。</div>`;
        return;
    }
        ...
        <div class="ci-card" data-id="${item.id}">
            <div class="ci-thumb-box">
                <div class="ci-thumb-preview-wrap" data-id="${item.id}" title="点击放大主图 (共 ${imgs.length} 张图片)">
                    <img class="ci-thumb-main ci-lazy-img" src="${displayUrl}" data-src="${displayUrl}" decoding="async" onclick="window.openCIModal('${displayUrl}')" />
        ...
        <div class="ci-panel">
            ...
            <div class="ci-list-header">
                ...
                <div class="ci-header-actions">
                    ...
                    <button id="ci-optimize-btn" class="menu_button ci-mini-btn" style="background:#3a86ff !important; color:#fff;" title="一键无损整理规则、去重并消除冗余字段">⚡ 优化库配置</button>
    ...
    document.getElementById('ci-export-btn').onclick = async () => {
        ...
        progress.update(0, total, `[2/2] ⚡ 正在生成标准化 JSON (0/${total})`);
        ...
        for (let i = 0; i < total; i++) {
            ...
            progress.update(i + 1, total, `[2/2] ⚡ 正在生成标准化 JSON (${i + 1}/${total})`);
    ...
    importInput.onchange = async (e) => {
        ...
        try {
            progress.update(0, 100, '准备导入数据...');
            clearOldIllustrations();
            urlManager.clearCache();

            importedCount = await parseAndImportJsonFile(file, charId, (loaded, total, statusText) => {
            ...
            progress.update(100, 100, '正在刷新界面...');
            await renderSettings();
            debouncedScan();
...
jQuery(async () => {
    ...
    // 延时二次重试渲染，防止酒馆初次加载 context.characters 尚未就绪导致查空
    setTimeout(() => {
        renderSettings();
        debouncedScan();
    }, 500);
