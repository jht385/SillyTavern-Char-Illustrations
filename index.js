...
// ==================== 0. 图片压缩引擎 (Blob/ObjectURL + 透明通道支持) ====================
// 使用 ObjectURL 映射替代巨型 Base64 字符串常驻内存
...
/**
 * 图像动态按需压缩与尺寸限制 (保留透明通道，输出 ObjectURL 释放内存)
 * @param {string} src 原图源
 * @param {number} quality 压缩质量 (0.1 ~ 1.0)
 * @param {number|null} maxDimension 最大边长像素
 * @returns {Promise<string>} 返回轻量 ObjectURL 指针
 */
function compressImage(src, quality = 0.8, maxDimension = null) {
    if (!src || typeof src !== 'string' || (!src.startsWith('data:image') && !src.startsWith('http') && !src.startsWith('/') && !src.startsWith('blob:'))) {
        return Promise.resolve(src);
    }
    const cacheKey = `${src.slice(0, 80)}_${src.length}_q${quality}_d${maxDimension || 'orig'}`;
    if (blobUrlCache.has(cacheKey)) {
        return Promise.resolve(blobUrlCache.get(cacheKey));
    }
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            // 转为 Blob 二进制对象并生成轻量指针 URL，避免大字符串撑爆 JS 堆
            canvas.toBlob((blob) => {
                if (!blob) {
                    resolve(src);
                    return;
        ...
        lazyObserver = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    const quality = parseFloat(img.dataset.quality) || 0.7;
                    const maxDim = parseInt(img.dataset.maxDim, 10) || 420;

                    if (rawSrc) {
                        compressImage(rawSrc, quality, maxDim).then(url => {
                            img.src = url;
            ...
            rootMargin: '150px 0px 150px 0px' // 提前 150px 预加载，无感滑动
        });
    }
    return lazyObserver;
}
...
function openImagePickerModal(item, onSelect) {
    ...
    const itemsHtml = imgs.map((rawSrc, idx) => {
        ...
        return `
            <div class="ci-picker-card ${isActive ? 'active' : ''}" data-idx="${idx}">
                <div class="ci-picker-img-box">
                    <img class="ci-lazy-img" data-src="${rawSrc}" data-quality="0.7" data-max-dim="420" />
    ...
    // 挂载完毕后通过 IntersectionObserver 按可视区懒加载图片
...
async function renderSettings() {
    ...
    // 骨架生成：缩略图挂载 data-src 由 IntersectionObserver 驱动，消除进设置面板卡死
        ...
        <div class="ci-card" data-id="${item.id}">
            <div class="ci-thumb-box">
                <div class="ci-thumb-preview-wrap" data-id="${item.id}" title="点击放大主图 (共 ${imgs.length} 张图片)">
                    <img class="ci-thumb-main ci-lazy-img" data-src="${rawSrc}" data-quality="0.6" data-max-dim="120" onclick="window.openCIModal('${rawSrc}')" />
