...
const urlManager = new ImageUrlManager(200);
...
let topFixedSize = localStorage.getItem('ci_top_fixed_size') || 'large'; // 'large' 或 'compact'
    ...
    for (const item of listToDisplay) {
        ...
        if (singleImageMode) {
            ...
            if (displayPosition === 'top_fixed') {
                ...
                card.appendChild(sizeToggleBtn);
            }
...
jQuery(async () => {
    ...
    renderSettings();
    debouncedScan();
    setTimeout(() => { renderSettings(); debouncedScan(); }, 500);
    setTimeout(() => { renderSettings(); debouncedScan(); }, 1500);
});