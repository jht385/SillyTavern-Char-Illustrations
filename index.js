...
async function openImagePickerModal(item, onSelect) {
    ...
    // 批量生成降低分辨率与质量的缩略图 (限制在 300px 以内，质量 0.6) 彻底解决手机端卡顿
    const lowResImgs = await Promise.all(imgs.map(src => compressImage(src, 0.6, 300)));
