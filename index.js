...
function getLazyObserver() {
    if (!lazyObserver) {
        lazyObserver = new IntersectionObserver((entries, observer) => {
            ...
            rootMargin: '200px 0px 200px 0px' // 提前 200px 预载
        });
    }
    return lazyObserver;
}
...
function openImagePickerModal(item, onSelect) {
    ...
    pickerModal.querySelectorAll('.ci-picker-card').forEach(card => {
        card.onclick = () => {
            ...
            // ⚡ 核心优化：点击立刻关闭弹窗，执行乐观UI无缝切换，彻底消除卡顿感
            closePicker();
            if (onSelect) {
                onSelect(chosenIdx);
            }
    ...
    for (const item of listToDisplay) {
        ...
        if (singleImageMode) {
            ...
            if (imgs.length > 1) {
                ...
                switchBtn.onclick = (e) => {
                    e.stopPropagation();
                    // ⚡ 延迟1帧渲染弹窗，使按钮按压动效与触摸反馈零延迟
                    requestAnimationFrame(() => {
                        openImagePickerModal(item, (chosenIdx) => {
                            item.selectedIndex = chosenIdx;
                            const newOriginal = imgs[chosenIdx];

                            // ⚡ 核心提速：秒切新图并立刻响应点击，绝不让用户等待 Canvas 编码
                            img.onclick = () => window.openCIModal(newOriginal);
                            
                            // 异步离屏生成 80% 压缩图后静默替换，并在后台异步刷入数据库
                            compressImage(newOriginal, 0.8).then(newCompressed => {
                                img.src = newCompressed;
                            });
                            
                            // 数据库异步排队持久化，完全移出UI事件链
                            setTimeout(() => {
