...
async function dbOptimizeCurrentChar(charId, onProgress) {
    const items = await dbGetIllustrations(charId);
    if (!items.length) return { totalRules: 0, savedDuplicates: 0, oldBytes: 0, newBytes: 0 };
    const db = await initDB();

    const blobRegistry = new Map();
    let optimizedItems = [];
    let savedDuplicates = 0;
    let totalImages = items.reduce((acc, it) => acc + (it.images?.length || 1), 0);
    ...
    for (let index = 0; index < items.length; index++) {
        ...
        for (const rawImg of rawImgs) {
            processedImages++;
            if (onProgress) {
                onProgress(processedImages, totalImages, `正在高保真无损整理 (${processedImages}/${totalImages})...`);
            }
...
async function renderSettings(forceRefreshStats = false) {
    ...
    if (optimizeBtn) {
        optimizeBtn.onclick = async (e) => {
            ...
            const progress = createProgressModal('⚡ 正在进行原画级保真整理');
            ...
            try {
                ...
                const oldMB = (res.oldBytes / 1024 / 1024).toFixed(1);
                const newMB = (res.newBytes / 1024 / 1024).toFixed(1);

                progress.close(500);
                toastr?.success?.(
                    `整理完成！当前画质完全无损保留，已规整 ${res.totalRules} 条规则，合并去重 ${res.savedDuplicates} 处数据。`,
                    '插图插件'
                );
            } catch (err) {
                progress.close(0);
