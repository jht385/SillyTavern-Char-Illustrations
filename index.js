...
// ==================== 0. 图片压缩与动态缩放引擎 (支持透明通道) ====================
const imageCompressCache = new Map();

/**
 * 图像动态按需压缩与尺寸限制 (保留透明通道)
 * @param {string} src 原图源 (Base64 或 URL)
 * @param {number} quality 压缩质量 (0.1 ~ 1.0)
 * @param {number|null} maxDimension 最大边长像素
 * @returns {Promise<string>}
 */
function compressImage(src, quality = 0.8, maxDimension = null) {
    if (!src || typeof src !== 'string' || (!src.startsWith('data:image') && !src.startsWith('http') && !src.startsWith('/'))) {
        return Promise.resolve(src);
    }
    const cacheKey = `${src.slice(0, 100)}_${src.length}_q${quality}_d${maxDimension || 'orig'}`;
    if (imageCompressCache.has(cacheKey)) {
        return Promise.resolve(imageCompressCache.get(cacheKey));
    }
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            try {
                // 使用 image/webp 导出：既能有高压缩率，又能完美支持透明通道
                let output = canvas.toDataURL('image/webp', quality);
                // 兼容性降级检查：若环境不支持 webp 导出而回退到 png，则直接使用
                if (!output.startsWith('data:image/webp') && !output.startsWith('data:image/png')) {
                    output = canvas.toDataURL('image/png');
                }

                if (imageCompressCache.size > 250) {
                    const firstKey = imageCompressCache.keys().next().value;
                    imageCompressCache.delete(firstKey);
                }
                imageCompressCache.set(cacheKey, output);
                resolve(output);
            } catch (err) {
                resolve(src);
            }
...
// ==================== 5. “更换插图”交互弹窗 (质量回调10%，由0.6升至0.7，分辨率适度放宽) ====================
async function openImagePickerModal(item, onSelect) {
    ...
    // 质量回调至 0.7 (原本 60% 回调 10% 到 70%)，边长适度调整为 420px，兼顾画质与流畅度
    const lowResImgs = await Promise.all(imgs.map(src => compressImage(src, 0.7, 420)));

    const itemsHtml = lowResImgs.map((thumbSrc, idx) => {
    ...
    const closePicker = () => {
        ...
        setTimeout(() => pickerModal.remove(), 200);
    };
...
function openManageImagesModal(item, onUpdated) {
            ...
            return `
                <div class="ci-manage-item ${isSelected ? 'is-selected' : ''}" data-idx="${idx}">
                    <div class="ci-manage-img-wrap">
                        ...
                        <img src="${src}" loading="lazy" title="点击放大预览" onclick="window.openCIModal('${src}')" />
                    </div>
    ...
    const closeManageModal = () => {
        ...
        setTimeout(() => manageModal.remove(), 200);
    };
...
async function renderSettings() {
    ...
    // 规则列表的小图标：压缩至 120px，质量 60%，透明背景完好保留
    const itemCardsHtml = await Promise.all(filteredItems.map(async (item, index) => {
        const imgs = item.images || (item.data ? [item.data] : []);
        const selectedIdx = typeof item.selectedIndex === 'number' && item.selectedIndex < imgs.length ? item.selectedIndex : 0;
        const rawSrc = imgs[selectedIdx] || imgs[0] || '';
        const names = item.imageNames || [];
        const currentName = names[selectedIdx] || '';

        const previewThumb60 = await compressImage(rawSrc, 0.6, 120);
        ...
        <div class="ci-card" data-id="${item.id}">
            <div class="ci-thumb-box">
                <div class="ci-thumb-preview-wrap" data-id="${item.id}" title="点击放大主图 (共 ${imgs.length} 张图片)">
                    <img class="ci-thumb-main" src="${previewThumb60}" loading="lazy" onclick="window.openCIModal('${rawSrc}')" />
    ...
    fileInput.onchange = async (e) => {
        ...
        for (const file of files) {
            ...
            thumb.onclick = () => window.openCIModal(b64);
    ...
    saveBtn.onclick = async () => {
        ...
        if (!tempBase64List.length) return alert('请先选择至少一张图片！');

        const rules = parseRuleInput(kwRaw);
        if (!rules.length) return alert('规则解析为空，请重新输入！');

        await dbAddIllustration(charId, rules, tempBase64List, new Array(tempBase64List.length).fill(''));
        tempBase64List = [];
        fileInput.value = '';
        countHint.innerText = '';
        renderSettings();
    ...
    document.getElementById('ci-export-btn').onclick = async () => {
        ...
        const exportData = {
            ...
            illustrations: items.map(item => ({
                rules: item.rules,
                images: item.images || (item.data ? [item.data] : []),
    ...
    importInput.onchange = async (e) => {
        ...
        try {
            ...
            for (const item of json.illustrations) {
                const imgs = item.images || (item.data ? [item.data] : []);
                if (imgs.length && item.rules) {
                    await dbAddIllustration(
                        charId,
                        item.rules,
                        imgs,
                        item.imageNames || [],
...
async function checkAndRenderMessage(messageEl) {
    ...
    for (const item of listToDisplay) {
        ...
        if (singleImageMode) {
            ...
            const originalImgSrc = imgs[selectedIdx];

            // 聊天气泡立绘：保持 80% 质量压缩，保留透明背景通道
            const compressed80 = await compressImage(originalImgSrc, 0.8);
            ...
            img.src = compressed80;
            img.loading = 'lazy';
            img.title = `点击放大查看\n触发规则: ${formatRuleDisplay(item)}`;
            // 点击全屏灯箱依然展示无损原图
            img.onclick = () => window.openCIModal(originalImgSrc);
            ...
            if (imgs.length > 1) {
                ...
                switchBtn.onclick = (e) => {
                    e.stopPropagation();
                    openImagePickerModal(item, async (chosenIdx) => {
                        item.selectedIndex = chosenIdx;
                        const newOriginal = imgs[chosenIdx];
                        const newCompressed80 = await compressImage(newOriginal, 0.8);
                        img.src = newCompressed80;
                        img.onclick = () => window.openCIModal(newOriginal);
                        await dbSetIllustrationSelectedIndex(item.id, chosenIdx);
                    });
                };
            ...
            for (const originalImgSrc of imgs) {
                const compressed80 = await compressImage(originalImgSrc, 0.8);
